...
public class SpringCrudifyJwtTokenProvider implements ISpringCrudifyAuthorizationProvider {
	...
	private void init() {
		...
		if( this.keyLifetime == 0) {
			...
			SpringCrudifyKeyExpiration expiration = new SpringCrudifyKeyExpiration(this.keyLifetime, this.keyLifetimeUnit);
			this.keyManager.createRealm(this.keyRealm, SignatureAlgorithm.forName(this.keyAlgorythm), expiration);
		}
	}
	
	@Override
	public String getAuthorization(Authentication authentication) {
    ...
    public String generateToken(String userName, String tenantId, String uuid){
